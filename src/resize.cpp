...
void ImageResize_optimized(const cv::Mat &src, cv::Mat &dst, const cv::Size sz)
{
    ...
    const int src_rows = src.rows;
    const int src_cols = src.cols;
    CV_Assert(src_cols == sz_src.width);
    CV_Assert(src_rows == sz_src.height);

    const int dst_rows = sz.height;
    const int dst_cols = sz.width;

    const float xscale = ((float)src_cols) / dst_cols;
    const float yscale = ((float)src_rows) / dst_rows;

    const float x0 = .5f * (xscale - 1);
    const float y0 = .5f * (yscale - 1);

    CV_Assert(src_cols >= dst_cols);
    CV_Assert(src_rows >= dst_rows);
    for (int row = 0; row < dst_rows; row++)
    {
        uchar *ptr_dst = dst.ptr<uchar>(row);
