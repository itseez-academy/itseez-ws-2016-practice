...
void ImageResize_optimized(const cv::Mat &src, cv::Mat &dst, const cv::Size sz)
{
	...
	float ratioX = (float)sz_src.width  / sz.width;
	float ratioY = (float)sz_src.height / sz.height;
	float offsetX = 0.5f*ratioX - 0.5f;
	float offsetY = 0.5f*ratioY - 0.5f;
	...
	if (src_rows > dst_rows && src_cols > dst_cols)
	{
		...
		{
			uchar *ptr_dst = dst.ptr<uchar>(row);
			const float y = row*ratioY + offsetY;
			const int iy = (y<0) ? (int)floor(y) : (int)y;
			const int y1 = iy;
			const int y2 = iy+1;

			for (int col = 0; col < dst_cols; col++)
			{
				const float x = col*ratioX + offsetX;
				const int ix = (x<0) ? (int)floor(x) : (int)x;

				const int x1 = ix;
				const int x2 = ix+1;

				const int q11 = src.at<uchar>(y1, x1);
				const int q12 = src.at<uchar>(y2, x1);
				const int q21 = src.at<uchar>(y1, x2);
				const int q22 = src.at<uchar>(y2, x2);
				...
				ptr_dst[col] = temp;//(temp < 0) ? 0 : ((temp > 255) ? 255 : (uchar)temp);
			}
		}
	}
	else
