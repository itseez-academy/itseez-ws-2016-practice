...
void ImageResize_optimized_1(const cv::Mat &src, cv::Mat &dst, const cv::Size sz)
{
    CV_Assert(CV_8UC1 == src.type());
    cv::Size sz_src = src.size();
    dst.create(sz, src.type());
    ...
    for (int row = 0; row < dst_rows; row++)
    {
        ...
        for (int col = 0; col < dst_cols; col++)
        {
            float x = col * xscale + x0;
			float y = row * yscale + y0;
            ...
            uchar q11 = src.at<uchar>(y1, x1);
            uchar q12 = src.at<uchar>(y2, x1);
            uchar q21 = src.at<uchar>(y1, x2);
            uchar q22 = src.at<uchar>(y2, x2);

            int temp = ((x1 == x2) && (y1 == y2)) ? (int)q11 :
              ( (x1 == x2) ? (int)(q11 * (y2 - y) + q22 * (y - y1)) :
              ( (y1 == y2) ? (int)(q11 * (x2 - x) + q22 * (x - x1)) : 
              (int)(q11 * (x2 - x) * (y2 - y) + q21 * (x - x1) * (y2 - y) + q12 * (x2 - x) * (y - y1) + q22 * (x - x1) * (y - y1))));

			//printf("(x,y)=(%f,%f)\t(ix,iy)=(%i,%i)\t(x1,y1)=(%i,%i)\t(x2,y2)=(%i,%i)\n", x, y, ix, iy, x1, y1, x2, y2);
