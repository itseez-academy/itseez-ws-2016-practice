...
void ImageResize_optimized(const cv::Mat &src, cv::Mat &dst, const cv::Size sz)
{
    ...
    for (int row = 0; row < dst_rows; row++)
    {
        ...
        for (int col = 0; col < dst_cols; col++)
        {
            const float x = (col + coeff) * width - coeff;
            const float y = (row + coeff) * height - coeff;

            const int ix = (int)(x);
            const int iy = (int)(y);

            const int x1 = (ix < 0) ? 0 : ix;
            const int x2 = (ix < 0) ? 0 : ix + 1;
            const int y1 = (iy < 0) ? 0 : iy;
            const int y2 = (iy < 0) ? 0 : iy + 1;
			...
			const int temp = (int)(q11 * (x2 - x) * (y2 - y) + q21 * (x - x1) * (y2 - y) + q12 * (x2 - x) * (y - y1) + q22 * (x - x1) * (y - y1));
