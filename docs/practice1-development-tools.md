...
__Цель данной работы__ — ознакомиться с современным рабочим процессом
(workflow) и используемыми в нем инструментами. Конкретно будут рассмотрены
...
__Основные задачи__
  ...
  1. __GitHub + Travis-CI__. Познакомиться с базовыми операциями GitHub в
     связке с Travis-CI: создание форка, создание pull request, контроль
     корректности на Travis-CI.
  1. __CMake + MSVS__. Научиться строить проект, реализованный с использованием
     CMake. Освоить основные операции с кодом: запуск демо-приложения и сборок
     с тестами.
...
## Общая последовательность действий
  ...
  1. Собрать проект и проверить его работоспособность, запустив тесты и
     демо-приложение (см. раздел [Сборка проекта с помощью CMake и MS
     VS][cmake-msvs]). Отдельно стоит запустить приложение с опцией `--save`, и
     проанализировать сохраненные изображения.
  1. Создать новую ветку в Git для выполнения практического задания (раздел
     [Общие инструкции по работе с Git][git-intro]).
  1. Реализовать несколько модульных тестов. После реализации каждого теста
     следует пересобрать проект и проверить работоспособность тестов. Изменения
     необходимо постоянно фиксировать, выкладывая в рабочую ветку на GitHub
     (раздел [Общие инструкции по работе с Git][git-intro]) и контролируя
     успешность тестирования на Travis-CI. К реализации предлагаются следующие
     тесты:
     1. Для функций `ConvertColor_BGR2GRAY_BT709` и `GuoHallThinning` нужно
        проверить, что выходная картинка имеет такие же размеры, как и
        исходная.
  ...
  1. Если останется время, можете заняться дополнительными задачами.
...
необходимо выполнить перед тем, как начать работать с проектом.
  ...
  1. Создать свой личный аккаунт на [GitHub][github.com], если таковой
     отсутствует. Для определенности обозначим аккаунт `github-account`.

  1. Сделать fork [центрального репозитория][ustream] (в терминологии Git
     upstream-репозиторий) к себе в личное пространство на GitHub под названием
...
примере ветки `add-new-google-tests`.
  ...
  #     upstream - репозиторий itseez-academy/itseez-ws-2016-practice
  ```
...
## Детальная инструкция по выполнению работы
  ...
  1. Создать рабочую ветку `add-new-google-tests` и перейти в нее для размещения
     реализаций фильтров (подробные инструкции в разделе
     [Общие инструкции по работе с Git][git-intro]).
  1. Сделать Pull Request в upstream-репозиторий, чтобы проверить
  ...
  1. При реализации каждого теста следует пересобирать проект и проверять
     ...
     выкладывая в рабочую ветку на сервер (разделы
     [Общие инструкции по работе с Git][git-intro],
     [Сборка проекта с помощью CMake и MS VS][cmake-msvs]).
  1. После того, как все тесты будут реализованые, вы можете выбрать и решить
     одну из задач списка [Дополнительные задачи][tasks].
