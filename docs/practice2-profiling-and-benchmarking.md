...
для выполнения второго практического задания.
  ...
  $ git pull upstream master  # Вливаем измнения из центрального репозитория
  $ git checkout -b profiling-and-benchmarking  # Создаем новую ветку для работы
  ```
...
создать ее заново:
  ...
  $ git remote update           # Получаем обновления из удаленного репозитория
...
Ну и на самом деле при желании можно вообще не задействовать ветку `master`:
  ...
  $ cd <itseez-ws-2016-practice>
  $ git remote -v     # Проверяем что репозиторий upstream сконфигурирован
  $ git remote update # Получаем актуальные версии кодов
...
затем анализировать их при помощи вспомогательных скриптов.
  ...
  1. Следующий скрипт, `summary.py`, позволяет сравнивать между собой несколько
     ...
     простоты просто запустим сборку `perf_skeleton` несколько раз, и сравним
     отчеты между собой.
